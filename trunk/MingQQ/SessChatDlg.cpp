...
extern CFontInfo g_buddyFontInfo;
...
BOOL CSessChatDlg::OnInitDialog(CWindow wndFocus, LPARAM lInitParam)
{
	...
	if (m_strGroupSig.IsEmpty())
	{
		if (m_lpQQClient != NULL)
			m_lpQQClient->UpdateC2CMsgSignal(m_nGroupId, m_nQQUin);
	}
...
// “字体名称”组合框
void CSessChatDlg::OnCbo_SelChange_FontName(UINT uNotifyCode, int nID, CWindow wndCtl)
{
	CString strText;
...
// “字体大小”组合框
void CSessChatDlg::OnCbo_SelChange_FontSize(UINT uNotifyCode, int nID, CWindow wndCtl)
{
	CString strText;
...
// “加粗”复选框
void CSessChatDlg::OnChk_Bold(UINT uNotifyCode, int nID, CWindow wndCtl)
{
...
// “斜体”复选框
void CSessChatDlg::OnChk_Italic(UINT uNotifyCode, int nID, CWindow wndCtl)
{
...
// “下划线”复选框
void CSessChatDlg::OnChk_UnderLine(UINT uNotifyCode, int nID, CWindow wndCtl)
{
...
// “颜色”按钮
void CSessChatDlg::OnBtn_Color(UINT uNotifyCode, int nID, CWindow wndCtl)
{
...
void CSessChatDlg::OnBtn_Send(UINT uNotifyCode, int nID, CWindow wndCtl)
{
	...
	if (!m_strGroupSig.IsEmpty())
	{
		m_lpQQClient->SendSessMsg(m_nQQUin, 0, m_strGroupSig, strText, 
			m_fontInfo.m_strName.c_str(), m_fontInfo.m_nSize, 
			m_fontInfo.m_clrText, m_fontInfo.m_bBold, 
			m_fontInfo.m_bItalic, m_fontInfo.m_bUnderLine);
	}
	else
	{
		m_arrPengingMsg.push_back(strText);
	}
...
void CSessChatDlg::OnUpdateC2CMsgSig()
{
	UpdateData();

	if (m_lpQQClient != NULL)
	{
		for (int i = 0; i < (int)m_arrPengingMsg.size(); i++)
		{
			m_lpQQClient->SendSessMsg(m_nQQUin, 0, m_strGroupSig, m_arrPengingMsg[i], 
				m_fontInfo.m_strName.c_str(), m_fontInfo.m_nSize, 
				m_fontInfo.m_clrText, m_fontInfo.m_bBold, 
				m_fontInfo.m_bItalic, m_fontInfo.m_bUnderLine);
		}
...
void CSessChatDlg::UpdateData()
{
	...
	if (lpBuddyInfo != NULL)
	{
		...
		m_strGroupSig = lpBuddyInfo->m_strGroupSig.c_str();
	}
...
void CSessChatDlg::UpdateGroupNameCtrl()
{
	...
	m_edtGroupName.SetWindowText(strText);
}

// 初始化控件
BOOL CSessChatDlg::InitCtrls()
{
	...
	m_edtGroupName = GetDlgItem(ID_EDIT_SIGN);
...
void CSessChatDlg::AddMsg(CSessMessage * lpSessMsg)
{
	...
	if (IsToday(lpSessMsg->m_nTime))
		strTime = FormatTime(lpSessMsg->m_nTime, _T("%H:%M:%S"));
	else
		strTime = FormatTime(lpSessMsg->m_nTime, _T("%Y-%m-%d %H:%M:%S"));
...
// "/f[表情id]/c[自定义表情文件名]"
void CSessChatDlg::AddMsg(LPCTSTR lpText)
{
	CString strTemp;
	int nLen;

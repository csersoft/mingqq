...
void CFaceCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (nIndex != -1)
	{
		...
		if (lpFaceInfo != NULL)
			::SendMessage(GetParent(), FACE_CTRL_SEL, 0, lpFaceInfo->m_nId);
	}
}
...
BOOL CFaceCtrl::LoadImage(int nPageIndex)
{
	...
	for (int i = nStart; i < nEnd; i++)
	{
		...
		if (lpFaceInfo != NULL)
		{
			...
			if (lpImage != NULL)
			{
				strFileName = strPath + lpFaceInfo->m_strFileName;
				BOOL bRet = lpImage->LoadFromFile(strFileName.c_str());
				if (bRet)

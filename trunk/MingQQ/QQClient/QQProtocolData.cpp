...
BOOL CVerifyCodeInfo::Parse(CBuffer * lpBuf)
{
	...
	//WCHAR * lpRespDataW = Utf8ToUnicode((const CHAR *)"ptui_checkVC(\'0\',\'!D9Y\', \'\\x00\\x00\\x00\\x00\\x32\\x87\\x00\\x6c\');");
	if (NULL == lpRespDataW)
		return FALSE;
	...
	{
		m_strVCType = _T("");
		m_strVerifyCode = cTemp1;
		m_nPtUinLen = sizeof(m_cPtUin) / sizeof(CHAR);
		ParsePtUin(cTemp2, m_cPtUin, m_nPtUinLen);
	}
	else
	{
		m_strVCType = cTemp1;
...
{
	if (NULL == lpInPtUin || NULL == *lpInPtUin || NULL == lpOutPtUin)
		return FALSE;
...
BOOL CLoginResult_1::Parse(CBuffer * lpBuf, std::vector<tstring>* arrRespHeader)
{
	...
	while (1)
	{
		...
		nPos1 = strCookie.find(_T("ptwebqq="));
		if (nPos1 != tstring::npos)
		{
			nPos2 = strCookie.find(_T(";"), nPos1+8);
			if (nPos2 != tstring::npos)
			{
				m_strPtWebQq = strCookie.substr(nPos1+8, nPos2 - nPos1 - 8);
				break;
			}
		}
...
// 获取HTTP头指定字段名称的值
tstring CLoginResult_1::GetRespHeader(std::vector<tstring>* arrRespHeader, 
...
BOOL CLoginResult_2::Parse(CBuffer * lpBuf)
{
...
CBuddyInfo * CGroupList::GetGroupMember(UINT nGroupCode, UINT nQQUin)
{
...
{
	if (_tcsicmp(lpType, _T("message")) == 0)
		return TRUE;
	else
		return FALSE;
}
...
BOOL CBuddyMessage::Parse(Json::Value& JsonValue)
{
	...
	for (int i = 0; i < nCount; ++i)
	{
		if (!JsonValue["content"][i].isNull())
		{
			...
			if (lpContent != NULL)
			{
				if (JsonValue["content"][i].isString())
				{
				...
				else if (JsonValue["content"][i].isArray())
				{
					...
					{
						strValue = JsonValue["content"][i][(UINT)0].asString();
						if (strValue == "font")
						{
							lpContent->m_nType = CONTENT_TYPE_FONT_INFO;

							if (!JsonValue["content"][i][(UINT)1]["size"].isNull())
								lpContent->m_FontInfo.m_nSize = JsonValue["content"][i][(UINT)1]["size"].asInt();

							if (!JsonValue["content"][i][(UINT)1]["color"].isNull())
							{
							...
							if (!JsonValue["content"][i][(UINT)1]["name"].isNull())
							{
								...
								lpContent->m_FontInfo.m_strName = AnsiToUnicode(strValue);
							}
						}
						else if (strValue == "face")
						{
...
// "/o[字体名称，大小，颜色，加粗，倾斜，下划线]/f[表情id]/c[自定义表情文件名]"
void CBuddyMessage::SetFormatString(LPCTSTR lpszMsg)
{
	tstring strTemp;
	CContent * lpContent;
	int nLen;

	if (NULL == lpszMsg || NULL == *lpszMsg)
		return;

	nLen = (int)_tcslen(lpszMsg);
	for (int i = 0; i < nLen; i++)
	{
		if (lpszMsg[i] == _T('/'))
		{
			...
			{
				if (lpszMsg[i+1] == _T('o') && lpszMsg[i+2] == _T('[') && lpszMsg[i+3] != _T(']'))
				{
					const TCHAR * lpStart = &lpszMsg[i+2];
					const TCHAR * lpEnd = _tcschr(&lpszMsg[i+3], _T(']'));
					if (NULL == lpEnd)
					{
						strTemp += lpszMsg[i];
						continue;
					...
					LPCTSTR lpFontFmt = _T("[%s,%d,%u,%d,%d,%d]");
					TCHAR cFontName[512] = _T("宋体");
					int nFontSize = 9;
					COLORREF clrText = RGB(0,0,0);
					BOOL bBold = FALSE, bItalic = FALSE, bUnderLine = FALSE;

					_stscanf(cBuf, lpFontFmt, &cFontName, &nFontSize, 
						(UINT *)&clrText, &bBold, &bItalic, &bUnderLine);

					lpContent = new CContent;
					if (lpContent != NULL)
					{
						lpContent->m_nType = CONTENT_TYPE_FONT_INFO;
						lpContent->m_FontInfo.m_strName = cFontName;
				...
				else if (lpszMsg[i+1] == _T('f') && lpszMsg[i+2] == _T('[') && lpszMsg[i+3] != _T(']'))
				{
					const TCHAR * lpStart = &lpszMsg[i+2];
					const TCHAR * lpEnd = _tcschr(&lpszMsg[i+3], _T(']'));
					if (NULL == lpEnd)
...
// "/o[字体名称，大小，颜色，加粗，倾斜，下划线]/f[表情id]/c[自定义表情文件名]"
tstring CBuddyMessage::ToFormatString()
{
	LPCTSTR lpFontFmt = _T("/o[%s,%d,%u,%d,%d,%d]");
	...
	for (int i = 0; i < (int)m_arrContent.size(); i++)
	{
		...
		if (lpContent != NULL)
		{
			...
			case CONTENT_TYPE_FONT_INFO:
				{
					...
					wsprintf(cBuf, lpFontFmt, lpContent->m_FontInfo.m_strName.c_str(),
						lpContent->m_FontInfo.m_nSize, lpContent->m_FontInfo.m_clrText,
			...
			case CONTENT_TYPE_CUSTOM_FACE:
				{
					memset(cBuf, 0, sizeof(cBuf));
					wsprintf(cBuf, lpCFaceFmt, lpContent->m_strCFaceName.c_str());
					strMsg += cBuf;
				}
				break;
			}
...
{
	if (_tcsicmp(lpType, _T("group_message")) == 0)
		return TRUE;
	else
		return FALSE;
}
				...
				else if (JsonValue["content"][i].isArray())
				{
					if (!JsonValue["content"][i][(UINT)0].isNull())
					{
						...
						else if (strValue == "cface")
						{
							...
							if (!JsonValue["content"][i][(UINT)1].isNull())
							{
								if (JsonValue["content"][i][(UINT)1].isObject())
								{
									if (!JsonValue["content"][i][(UINT)1]["name"].isNull())
									{
										strValue = JsonValue["content"][i][(UINT)1]["name"].asString();
...
{
	if (_tcsicmp(lpType, _T("sess_message")) == 0)
		return TRUE;
	else
		return FALSE;
}
...
{
	if (_tcsicmp(lpType, _T("buddies_status_change")) == 0)
		return TRUE;
	else
		return FALSE;
}
...
{
	if (_tcsicmp(lpType, _T("kick_message")) == 0)
		return TRUE;
	else
		return FALSE;
}
...
{
	if (_tcsicmp(lpType, _T("sys_g_msg")) == 0)
		return TRUE;
	else
		return FALSE;
}

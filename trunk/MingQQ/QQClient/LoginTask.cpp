...
int CLoginTask::Run()
{
	...
	if (m_lpQQUser->m_strVerifyCode.empty())
	{
		...
		if (m_lpQQUser->m_VerifyCodeInfo.m_nNeedVerify)	// 需要输入验证码
		{
			...
			else
			{
				::PostMessage(m_lpQQUser->m_hCallBackWnd, 
					QQ_MSG_LOGIN_RESULT, NULL, QLRC_NEED_VERIFY_CODE);
				m_bStop = FALSE;
				return 0;
			}
		}
	...
	if (!bRet || m_lpQQUser->m_LoginResult1.m_nRetCode != 0 || m_bStop)
		goto Ret0;

	m_lpQQUser->m_strVerifyCode = _T("");

	if (m_lpQQUser->m_LoginResult1.m_nRetCode != 0)
	{
		if (m_lpQQUser->m_LoginResult1.m_nRetCode == 4)	// 验证码错误
		{
			...
			{
				::PostMessage(m_lpQQUser->m_hCallBackWnd, 
					QQ_MSG_LOGIN_RESULT, NULL, QLRC_VERIFY_CODE_ERROR);
				m_bStop = FALSE;
				return 0;
			}
		}
		...
		{
			::PostMessage(m_lpQQUser->m_hCallBackWnd, 
				QQ_MSG_LOGIN_RESULT, NULL, QLRC_PASSWORD_ERROR);
			m_bStop = FALSE;
			return 0;
		}
		else	// 未知错误
	...
	bRet = StartPollTask();
	bRet = StartSendMsgTask();
	bRet = StartGetBuddyNumberTask(lpBuddyListResult, lpGroupListResult);
	bRet = StartGetBuddySignTask(lpBuddyListResult);
	bRet = StartGetHeadPicTask(lpBuddyListResult, lpGroupListResult);

	if (m_bStop)
		goto Ret0;

	m_lpQQUser->m_nStatus = m_lpQQUser->m_LoginResult2.m_nStatus;		// 登陆成功
	::PostMessage(m_lpQQUser->m_hCallBackWnd, 
		QQ_MSG_LOGIN_RESULT, NULL, QLRC_SUCCESS);
	::PostMessage(m_lpQQUser->m_hCallBackWnd, 
...
Ret0:
	...
	if (m_bStop)
		::PostMessage(m_lpQQUser->m_hCallBackWnd, QQ_MSG_LOGIN_RESULT, NULL, QLRC_USER_CANCEL_LOGIN);
	else
		::PostMessage(m_lpQQUser->m_hCallBackWnd, QQ_MSG_LOGIN_RESULT, NULL, QLRC_FAILED);
	m_bStop = FALSE;
	return 0;
}
...
BOOL CLoginTask::StartPollTask()
{
	...
	m_lpPollTask->m_lpThreadPool = m_lpThreadPool;
...
// 		return FALSE;
// 
// 	lpQQNumTask->m_lpQQUser = m_lpQQUser;
// 	lpQQNumTask->m_lpQQProtocol = m_lpQQProtocol;
// 
// 	for (int i = 0; i < lpBuddyListResult->GetBuddyTeamCount(); i++)
// 	{
// 		CBuddyTeamInfo * lpBuddyTeamInfo = lpBuddyListResult->GetBuddyTeam(i);
...
// 				CBuddyInfo * lpBuddyInfo = lpBuddyTeamInfo->GetBuddy(j);
// 				if (lpBuddyInfo != NULL)
// 					lpQQNumTask->AddParam(OP_TYPE_GETBUDDYNUM, 0, lpBuddyInfo->m_nQQUin);
// 			}
// 		}	
// 	}
// 
...
// 		CGroupInfo * lpGroupInfo = lpGroupListResult->GetGroup(i);
// 		if (lpGroupInfo != NULL)
// 			lpQQNumTask->AddParam(OP_TYPE_GETGROUPNUM, lpGroupInfo->m_nGroupCode, 0);
// 	}
// 
// 	return m_lpThreadPool->AddTask(lpQQNumTask);
}
									 ...
									 CGroupListResult * lpGroupListResult)
{
	...
	if (IsNeedUpdateHeadPic(0, m_lpQQUser->m_nQQUin))
		lpGetHeadPicTask->AddParam(0, m_lpQQUser->m_nQQUin, 0, m_lpQQUser->m_nQQUin);
	...
	for (int i = 0; i < lpBuddyListResult->GetBuddyTeamCount(); i++)
	{
		...
		if (lpBuddyTeamInfo != NULL)
		{
			for (int j = 0; j < (int)lpBuddyTeamInfo->GetBuddyCount(); j++)
			{
				...
				if (lpBuddyInfo != NULL)
				{
					if (IsNeedUpdateHeadPic(0, lpBuddyInfo->m_nQQNum))
						lpGetHeadPicTask->AddParam(0, lpBuddyInfo->m_nQQUin, 0, lpBuddyInfo->m_nQQNum);
				}
			}
		}	
	}
	...
	for (int i = 0; i < lpGroupListResult->GetGroupCount(); i++)
	{
		...
		if (lpGroupInfo != NULL)
		{
			if (IsNeedUpdateHeadPic(lpGroupInfo->m_nGroupNumber, 0))
				lpGetHeadPicTask->AddParam(lpGroupInfo->m_nGroupCode, 0, lpGroupInfo->m_nGroupNumber, 0);
		}
	}
...
// 				CBuddyInfo * lpBuddyInfo = lpBuddyTeamInfo->GetBuddy(j);
// 				if (lpBuddyInfo != NULL)
// 					lpGetHeadPicTask->AddParam(0, lpBuddyInfo->m_nQQUin);
// 			}
// 		}	
// 	}
// 
...
// 		CGroupInfo * lpGroupInfo = lpGroupListResult->GetGroup(i);
// 		if (lpGroupInfo != NULL)
// 			lpGetHeadPicTask->AddParam(lpGroupInfo->m_nGroupCode, 0);
// 	}
// 
// 	return m_lpThreadPool->AddTask(lpGetHeadPicTask);
}

BOOL CLoginTask::IsNeedUpdateHeadPic(UINT nGroupNum, UINT nQQNum)
{
	tstring strFullName = m_lpQQUser->GetHeadPicFullName(nGroupNum, nQQNum).c_str();

...
int CChangeStatusTask::Run()
{
	...
	if (!bRet || ChangeStatusResult.m_nRetCode != 0)
		::PostMessage(m_lpQQUser->m_hCallBackWnd, QQ_MSG_CHANGE_STATUS_RESULT, FALSE, 0);
	else
		::PostMessage(m_lpQQUser->m_hCallBackWnd, QQ_MSG_CHANGE_STATUS_RESULT, TRUE, m_nStatus);

...
#define		QQ_MSG_UPDATE_BUDDY_HEADPIC		WM_USER + 121		// 更新好友头像
#define		QQ_MSG_UPDATE_GMEMBER_HEADPIC	WM_USER + 122		// 更新群成员头像
#define		QQ_MSG_UPDATE_GROUP_HEADPIC		WM_USER + 123		// 更新群头像
#define		QQ_MSG_CHANGE_STATUS_RESULT		WM_USER + 124		// 改变在线状态返回消息
...
private:
	tstring GetRespHeader(std::vector<tstring>* arrRespHeader, 
		LPCTSTR lpszName, int nIndex = 0);	// 获取HTTP头指定字段名称的值
...
public:
	...
	CBuddyInfo * GetGroupMember(UINT nGroupCode, UINT nQQUin);	// 根据群代码和群成员QQUin获取群成员信息
...
public:
	...
	tstring m_strText;
	int m_nFaceId;
	tstring m_strCFaceName;
	UINT m_nCFaceFileId;
	tstring m_strCFaceKey;
	tstring m_strCFaceServer;
...
public:
	...
	void SetFormatString(LPCTSTR lpszMsg);
	tstring ToFormatString();
